...
<p className="text-[1.5rem] ml-7 md:w-[500px] w-auto mt-5 animate ">Following projects showcases my skills and experience through
...
{projectdetails.map((project, index) => (
          <div key={index} className="md:w-[450px] w-auto h-auto flex mb-5 animate">
            <div className="mb-5 ">
              <div className="flex">
                <h1 className="text-[2rem] font-medium flex-1">{project.title}</h1>
                <ul className="flex justify-around flex-1 md:text-[1.5rem] md:mb-1 m-auto">
                  {project.technologies.map((tech, i) => (
                    <li key={i} className="bg-[#00ffee] p-2 rounded-md md:inline-block text-black font-medium cursor-pointer  ">
                      {tech}
                    </li>
                  ))}
                </ul>
              </div>
              <div className="border rounded-md md:w-[400px] w-auto ">
